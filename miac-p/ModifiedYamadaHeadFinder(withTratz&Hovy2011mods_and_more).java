...
public class ModifiedYamadaHeadFinder extends HeadFinder {
    ...
    public void setHeads(NonterminalNode node) {
        ...
        /*if(node.getLabel().equals("GLARF")) {
            ...
            if(!test)
                if(posAsHead)
                    test = trySetHead(node, end, "FW|NML|NN|NNP|NNPS|NNS|NX|POS", null, true);
                else
                    test = trySetHead(node, end, "FW|NML|NN|NNP|NNPS|NNS|NX", null, true);
...
//                || trySetHead(node, end, "QP", true);
            ...
            if(!test)
                ...
                || trySetHead(node, end, "CD|FW", null, true)
                || trySetHead(node, end, "QP", null, false)
                || trySetHead(node, end, "JJ|NAC", null, true)
                || trySetHead(node, end, "JJS", null, true)
                || trySetHead(node, end, "PRP", null, true)
                || trySetHead(node, end, "ADJP", null, true) 
                || trySetHead(node, end, "RBS|RBR", null, true) 
                || trySetHead(node, end, "VBG|DT|WP", null, true)
            ...
            if(!test)
                ...
                || trySetHead(node, end, "CD", true)
                || trySetHead(node, end, "JJ|NAC", true)
                || trySetHead(node, end, "JJS", true)
                || trySetHead(node, end, "RB", true)
                || trySetHead(node, end, "QP", false);
            
            if(!test) {
            	test = trySetHead(node, end, "S|SBAR|UCP|PP", false) || trySetHead(node, end, "SINV|SBARQ|SQ", false);
            }
            if(!test) {
            	test = trySetHead(node, end, "UH", false);
            }
            
            if(!test) {
            	test = trySetHead(node, end, "VP|NP", false);
            }
            if(!test) {
            	test = trySetHead(node, end, "VB|VBP", false);
            }
            ...
            || trySetHead(node, end, "QP|NN", true)
            //|| trySetHead(node, end, "NN", true)
            || trySetHead(node, end, "\\$|\\#", true) 
            || trySetHead(node, end, "JJ|JJR", true) // mod
            || trySetHead(node, end, "VBN|VBD", true) 
            ...
            || trySetHead(node, end, "JJS", true)
            || trySetHead(node, end, "DT|PDT", true) // what about PDT?
        	...
        	//test = trySetHead(node, end, "IN", true);        	
        		test = trySetHead(node, end, "RB|RBR|JJ|JJR", true)
        		//test = trySetHead(node, end, "RB|RBR", true)
                //|| trySetHead(node, end, "RBR", true)
                || trySetHead(node, end, "RBS", true)
            ...
            || trySetHead(node, end, "DT|NN|SYM", false)
            || trySetHead(node, end, ANY_LBL, false);
        ...
        */
            ...
            if(prepAsHead) {
                ...
                || trySetHead(node, end, "NN|NNS|NNP|NNPS|VBG|VBN|VBD|WDT|WP|WP\\$|WRB|VBZ|VBP|VB|TO|IN|RB|RBR|RBS|JJ|JJR|JJS|SYM|X|LS|CD|CC|PRP|PRP\\$|PDT|DT|POS|EX|RP|UH|NP|VP|S|SBAR|PP|SBARQ|UCP|NML|PRN|QP|RRC|NX|NAC|FRAG|INTJ|ADJP|ADVP|LST|WHADJP|WHAVP|WHNP|WHPP|X", false) 
                || trySetHead(node, end, "<<<ANY_LBL>>>", false);
            } else {
            ...
            || trySetHead(node, end, "NN|NNP|NNPS|NNS|NX|NML", true)
            ...
            || trySetHead(node, end, "RB", false);
            if(!test) {
            	test = trySetHead(node, end, "NAC", false);
            }
            if(!test) {
            	test = trySetHead(node, end, "VP", false);
            }
            if(!test) {
            	test = trySetHead(node, end, "INTJ", false);
            }
            if(!test) {
            	test = trySetHead(node, end, "CC", false) ||
            	trySetHead(node, end, "FRAG", false);
            }
            if(!test)
                for(Node child: node)
                    if(child instanceof NonterminalNode) {
		...
		node.setHeadChild(node.getChild(0));
            ...
            || trySetHead(node, end, "CD", true)
            || trySetHead(node, end, "JJ", true)
            || trySetHead(node, end, "RB|DT|PDT", true)
            //|| trySetHead(node, end, "DT", true)
            || trySetHead(node, end, "NCD", true)
            || trySetHead(node, end, "QP", true)
            || trySetHead(node, end, "IN", true) // moved down
            || trySetHead(node, end, "CC", false) 
            || trySetHead(node, end, "JJR|RBR", true)
            || trySetHead(node, end, "JJS", true)
            || trySetHead(node, end, ANY_LBL, true);
            ...
            */
            ...
            test = subAsHead ? 
                    trySetHead(node, end, "PREP_AUX", true) 
                    || trySetHead(node, end, "DT", true) /*|| trySetHead(node, end, "RB", true)*/:
                    false;
            ...
            if(!test)
                ...
                || trySetHead(node, end, "FRAG", true) 
                || trySetHead(node, end, "X", true)
                || trySetHead(node, end, ANY_LBL, true);

        } else if(node.getLabel().equals("UCP")) {
        	//test = trySetHead(node, end, ".*P|S.*|N.*|PRN|RRC|FRAG|INTJ|LST|X", false);
        	if(conjAsHead) {
        		test = trySetHead(node, end, "CC", false);
        	}
        	else {
        	test = trySetHead(node, end, "N[^N]+|VP|S.*|PP|UCP|PRN|QP|RRC|FRAG|INTJ|AD(J|V)P|LST|WH.*|X", false)
        								|| trySetHead(node, end, "[^D].*", false);
        	}

        	//test = trySetHead(node, end, "N.*|S.*|W.*|V.*|UH|PP|PRN|PRP|FRAG|INTJ|LST|X", false)
        	//|| trySetHead(node, end, "J.*|R.*", false)
        	//|| trySetHead(node, end, "CONJP", false)
        	//|| trySetHead(node, end, "[A-Z\\$#]*", false);
           if(!test) test = trySetHead(node, end, ANY_LBL, false);
            ...
            || trySetHead(node, end, "WRB|ADJP|JJP", true)
            ...
            test = trySetHead(node, end, "CC", false)
            || trySetHead(node, end, "WRB|RB|JJ|NN", true) 
            || trySetHead(node, end, ANY_LBL, false);
        ...
        /*else if(node.label.equals("WHPP")) {                   
        	...
        	//test = trySetHead(node, end, ".*P|S.*|N.*|PRN|RRC|FRAG|INTJ|LST|X", false);
        	test = trySetHead(node, end, "N[^N]+|VP|S.*|PP|UCP|PRN|QP|RRC|FRAG|INTJ|ADJP|ADVP|LST|WH.*|X", false);
            if(!test) test = trySetHead(node, end, ANY_LBL, true);
        }
