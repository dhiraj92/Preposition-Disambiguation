...
package miacp.runpipe.impl.annotators.pos;
		...
		if(allSentences != null && allTokens != null) {
			try {
				for(Sentence sentence : allSentences) {
					...
					List<miacp.parse.types.Token> gtokens = new ArrayList<miacp.parse.types.Token>(tokens.size());
					int index = 0;
					for(Token tok : tokens) {
						gtokens.add(new miacp.parse.types.Token(tok.getAnnotText(), ++index));
					}
