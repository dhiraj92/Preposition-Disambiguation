...
public class NLParser {
	...
	public NLParser(File modelFile) throws IOException, ClassNotFoundException {
	...
	public Parse parseSentence(Sentence sentence) {
		...
		while(first != null && first.next != null) {
			...
			if(bestAction != null) {
				first = performAction(tokens, first, tokenToPtr, bestAction, actionListStale, currentArcs, creationOrderIndex, mFeatGen.getContextWidth());
	...
	public static TokenPointer performAction(List<Token> sentence, 
											  ...
											  int actionNum,
											  int contextWidth) {
		int widthToReset = contextWidth/2;
		...
		if(action.actionName.equals(ParseConstants.SWAP_LEFT_ACTION_NAME)) {
			widthToReset++;
		}
		clearPtr = save.prev;
