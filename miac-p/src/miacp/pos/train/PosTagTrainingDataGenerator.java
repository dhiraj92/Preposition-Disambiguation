...
public class PosTagTrainingDataGenerator {
	...
	private static CommandLineOptions createOptions() {
		...
		options.addOption(OPT_SENTENCE_READER, "classname", "name of the sentence reader class (must implement " + miacp.parse.io.SentenceReader.class.getName() + ")");
		options.addOption(OPT_OUTPUT_FILE, "file", "the output file");
		options.addOption(OPT_FEAT_GEN, "classname", "name of the feature generation class (must implement " + miacp.pos.featgen.PosFeatureGenerator.class.getName() + ")");
	...
	public static void main(String[] args) throws Exception {
		...
		for(String dataFile : dataFiles.split(File.pathSeparator)) {
			...
			while((parse = sentenceReader.readSentence(reader)) != null) {
				...
				if(snum % 500 == 0) System.err.println(snum);
				snum++;
			}
			reader.close();
		}
