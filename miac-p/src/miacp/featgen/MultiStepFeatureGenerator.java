...
import miacp.featgen.CombinationEntry.ComboType;
import miacp.featgen.fer.FERProductionException;
...
public class MultiStepFeatureGenerator implements Serializable {
	...
	public Set<String> generateFeatures(List<Token> tokens, Parse parse, int wordIndex) {
		...
		Map<miacp.parse.types.Token, Map<FeatureExtractionRule, Set<String>>> tokenToFerToProductions 
		= new HashMap<Token, Map<FeatureExtractionRule, Set<String>>>();
		for(WfrEntry wfrEntry : mWfrEntries) {
			Set<miacp.parse.types.Token> wfrResults = wfrEntry.getWfrRule().getProductions(tokens, parse, wordIndex);
